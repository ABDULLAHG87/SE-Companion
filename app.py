...
from secompanion.apps.forms import (LoginForm, RegistrationForm, ProfileForm,
                                    ForgotPasswordForm)
from secompanion.apps.models import (User, db, Interest, Skill, Connection,
                                     Tool, Resource)
...
from secompanion.apps.methods import (send_password_reset_email,
                                      filter_resources, get_daily_quote)
...
app.config['SECRET_KEY'] = (
    b'8\xc6\xef\xd8\x82\xf86\xe5R\x10\xb3\x9f\xb8k\xf0{\x88-\xc4\xde\x8eQ\x05;'
)
basedir = os.path.abspath(os.path.dirname(__file__))
app.config['SQLALCHEMY_DATABASE_URI'] = (
    'mysql://root:abdul4prof@localhost/secompanion'
)
...
def dashboard():
    ...
    categories = ['success', 'productivity', 'leadership', 'inspiration',
                  'work', 'time', 'today', 'pain']
    selected_category = random.choice(categories)
    quote = get_daily_quote(selected_category)
    daily_quotes = quote if quote else "No quote found today."
    return render_template('/dashboard/dashboard.html',
                           username=current_user.username, quote=daily_quotes)
...
def profile():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if form.photo.data:  # Check if photo was uploaded
            ...
            form.photo.data.save(os.path.join(app.config['UPLOAD_FOLDER'],
                                              filename))
...
resources = [
    {"title": "Resource 1", "type": "Video", "category": "Web Development",
     "level": "Beginner"},
    {"title": "Resource 2", "type": "Book", "category": "Data Science",
     "level": "Intermediate"},
    {"title": "Resource 3", "type": "Article",
